#!/usr/bin/env bash
# modules/20-site.sh — vòng đời site: create / list / info / delete.

MODULE_NAME="Quản lý Website"
MODULE_ORDER=20
MODULE_COMMANDS=("site:create" "site:list" "site:info" "site:delete")

# --- Rollback stack: đẩy lệnh undo, chạy ngược khi fail ---
...
# Parse --key value vào biến ARG_<KEY>.
_parse_args() {
...
cmd_site_create() {
  ...
  local domain; domain="$(resolve_input "domain" "$ARG_DOMAIN" \
    --prompt "Domain (vd: app.example.vn)" --validate validate_domain)"
  state_site_exists "$domain" && die "Site '$domain' đã tồn tại."

  # 2) type
  local type; type="$(resolve_input "type" "$ARG_TYPE" \
    --prompt "Loại app" --select "nextjs express static" --validate validate_app_type)"

  # 3) node version (static không bắt buộc nhưng vẫn hỏi cho thống nhất)
  ...
  user="${user:0:32}"   # giới hạn độ dài tên user Linux
  local port
  if [[ -n "$ARG_PORT" ]]; then
    net_check_user_port "$ARG_PORT" || die "Port --port=$ARG_PORT không dùng được."
    port="$ARG_PORT"
  else
    port="$(net_alloc_port)" || die "Hết port trống trong range ${LAPN_PORT_MIN}-${LAPN_PORT_MAX}."
  fi

  # Cảnh báo DNS nếu chưa trỏ về server.
  _site_warn_dns "$domain"

  log_step "Tạo site $domain (type=$type, port=$port, user=$user)"

  # 5) user + thư mục + secrets + .env
  ...
  # symlink .env vào app dir
  ln -sf "$envfile" "${appdir}/.env"

  # 5b) DB (tùy chọn)
  if [[ -n "$ARG_DB" ]] || { [[ "${LAPN_INTERACTIVE:-0}" == "1" ]] && ui_confirm "Gắn database cho site?"; }; then
    _site_attach_db "$domain" "$name" "$user" "$envfile" "$ARG_DB"
  fi

  # 6) git clone + build (tùy chọn)
  local git_url="$ARG_GIT"
  if [[ -z "$git_url" && "${LAPN_INTERACTIVE:-0}" == "1" ]]; then
    git_url="$(ui_ask "Git repo (bỏ trống nếu deploy thủ công sau)")"
  fi
  if [[ -n "$git_url" ]]; then
    log_step "Clone $git_url"
    sudo -u "$user" git clone "$git_url" "$appdir" 2>/dev/null \
      || sudo -u "$user" bash -c "cd '$appdir' && git clone '$git_url' ." \
      || { _undo_run; die "git clone thất bại."; }
    # Cài Node cho user rồi build (npm ci + build nếu có script build).
    ...
    " || { _undo_run; die "Build thất bại."; }
  fi

  # 7) systemd unit (trừ static) + 8) health check
  if adapter_needs_unit; then
    ...
    systemctl enable --now "lapn-${name}.service" \
      || { _undo_run; die "Khởi động unit thất bại — xem: journalctl -u lapn-${name}"; }
    ...
    _site_health_check "$port" "$(adapter_health_url)" \
      || log_warn "Health check chưa pass — app có thể cần cấu hình thêm. Kiểm tra journalctl -u lapn-${name}"
  fi

  # 9) nginx conf
  _site_render_nginx "$domain" "$name" "$type" "$port" "$appdir"
  nginx -t || { _undo_run; die "nginx -t lỗi."; }
  ...
  # 10) SSL (tùy chọn, sau khi đã ghi state để ssl module đọc được)
  if [[ "${LAPN_INTERACTIVE:-0}" == "1" ]] && ui_confirm "Cài SSL ngay cho $domain?" Y; then
...
_site_warn_dns() {
  ...
  if [[ -n "$server_ip" && -n "$resolved" && "$server_ip" != "$resolved" ]]; then
    log_warn "DNS của $domain ($resolved) chưa trỏ về IP server ($server_ip). SSL HTTP-01 sẽ fail tới khi trỏ đúng."
  fi
}
...
_site_render_unit() {
  ...
  [[ -z "$exec_start" ]] && die "adapter không trả ExecStart cho $domain."
...
_site_render_nginx() {
  ...
  local cf_include=""   # mặc định không CF (site:create chưa bật CF)
...
_site_attach_db() {
  ...
  if [[ -z "$engine" ]]; then
    engine="$(resolve_input "engine" "" --prompt "Engine DB" \
      --select "mariadb postgres mongo redis" --validate validate_db_engine)"
  fi
  if ! declare -F cmd_db_create >/dev/null; then
    log_warn "Module DB chưa sẵn — bỏ qua bước gắn DB."
    return 0
  fi
  if ! state_service_installed "$engine"; then
    if ui_confirm "Engine '$engine' chưa cài. Cài ngay?" Y; then
      core_dispatch "db:install" "$engine"
    else
      log_warn "Bỏ qua gắn DB (engine chưa cài)."
      return 0
    fi
  fi
...
_site_summary() {
  ...
  printf '\n%s%s✓ Site %s đã sẵn sàng%s\n' "$C_GREEN" "$C_BOLD" "$domain" "$C_RESET"
  printf '  URL      : %s://%s\n' "$scheme" "$domain"
  printf '  Port nội : 127.0.0.1:%s\n' "$port"
  printf '  User     : %s\n' "$user"
  printf '  App dir  : %s\n' "$root"
...
cmd_site_list() {
  ...
  if [[ "$count" == "0" ]]; then log_info "Chưa có site nào."; return 0; fi
...
cmd_site_delete() {
  ...
  state_init
  local domain; domain="$(resolve_input "domain" "$ARG_DOMAIN" \
    --prompt "Domain cần xóa" --validate validate_domain)"
  state_site_exists "$domain" || die "Không có site '$domain'."
  ...
  if [[ -z "$ARG_FORCE" ]]; then
    local confirm
    confirm="$(ui_ask "Gõ lại đúng domain để xác nhận XÓA")"
    [[ "$confirm" == "$domain" ]] || die "Xác nhận không khớp — hủy."
  fi

  log_step "Xóa site $domain"

  # Drop DB của site (nếu có và module DB sẵn sàng).
  if declare -F cmd_db_drop >/dev/null; then
    local engines; engines="$(state_site_get "$domain" | jq -r '.db[]?.engine' 2>/dev/null || true)"
    local e
    for e in $engines; do
      cmd_db_drop --site "$domain" --engine "$e" --force || log_warn "Drop DB $e lỗi (bỏ qua)."
    done
  fi
  ...
  if command -v certbot >/dev/null 2>&1; then
    if [[ -n "$ARG_FORCE" ]] || ui_confirm "Thu hồi SSL cert của $domain?"; then
      certbot delete --cert-name "$domain" --non-interactive 2>/dev/null || true
    fi
  fi

  # backup nhanh ra trash
  mkdir -p "$LAPN_TRASH"
  local stamp; stamp="$(date +%Y%m%d-%H%M%S)"
  if [[ -d "$home" ]]; then
    tar -czf "${LAPN_TRASH}/${name}-${stamp}.tar.gz" -C "$LAPN_SITES_HOME" "$name" 2>/dev/null || true
    log_info "Backup nhanh: ${LAPN_TRASH}/${name}-${stamp}.tar.gz (giữ 7 ngày)"
  fi
  # Dọn trash > 7 ngày.
  find "$LAPN_TRASH" -name '*.tar.gz' -mtime +7 -delete 2>/dev/null || true

  # xóa user + home + secrets
  if id "$user" >/dev/null 2>&1; then
