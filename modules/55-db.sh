#!/usr/bin/env bash
# modules/55-db.sh — Database tùy chọn: cài từng engine, per-site user, bind localhost.
# Remote bằng Navicat qua SSH tunnel (db:remote). KHÔNG cài tool web.
...
cmd_db_install() {
  core_require_root
  local engine="${1:-}"
  [[ -z "$engine" ]] && engine="$(resolve_input "engine" "" --prompt "Engine cần cài" \
    --select "mariadb postgres mongo redis mysql" --validate validate_db_engine)"
  validate_db_engine "$engine" || die "Engine không hợp lệ."

  if state_service_installed "$engine"; then
    log_info "Engine '$engine' đã cài (idempotent — bỏ qua)."
    return 0
  fi
  ...
  log_ok "Đã cài $engine (bind 127.0.0.1:$port)."
}
...
_db_install_mariadb() {
  ...
  # Cứng hóa: đặt root password, xóa anonymous + test db.
...
FLUSH PRIVILEGES;
SQL
  log_ok "MariaDB root password lưu ở $(_db_root_secret mariadb)"
}
...
_db_install_postgres() {
  log_step "Cài PostgreSQL"
  apt-get install -y postgresql
  # listen localhost (mặc định đã localhost; ép cho chắc).
  ...
  log_ok "PostgreSQL nghe localhost."
}

_db_install_mongo() {
  log_step "Cài MongoDB (repo chính thức)"
  ...
  mongosh --quiet --eval "db.getSiblingDB('admin').createUser({user:'lapnadmin',pwd:'${rootpass}',roles:['root']})" 2>/dev/null \
    || log_warn "Tạo admin Mongo có thể đã tồn tại."
}
...
_db_install_redis() {
  ...
  log_ok "Redis requirepass lưu ở $(_db_root_secret redis)"
}
...
cmd_db_create() {
  ...
  local engine; engine="$(resolve_input "engine" "$DB_ENGINE" --prompt "Engine" \
    --select "mariadb postgres mongo redis mysql" --validate validate_db_engine)"
  state_service_installed "$engine" || die "Engine '$engine' chưa cài. Chạy: lapn db:install $engine"
  ...
  log_step "Tạo DB $engine cho $domain"
  local url=""
  case "$engine" in
  ...
  # Chèn biến kết nối vào .env (idempotent: xóa dòng cũ rồi thêm).
  if [[ -n "$url" ]]; then
  ...
  # Ghi state (engine/name/user — KHÔNG password).
  local dbinfo
  ...
  log_ok "DB '$dbname' (user '$dbuser') đã tạo, biến kết nối ghi vào .env."
}

_db_create_mysql() {
  local dbname="$1" dbuser="$2" dbpass="$3"
  mysql <<SQL || die "Tạo DB MySQL/MariaDB thất bại."
...
_db_create_postgres() {
  local dbname="$1" dbuser="$2" dbpass="$3"
  sudo -u postgres psql <<SQL || die "Tạo DB PostgreSQL thất bại."
DO \$\$ BEGIN
...
_db_create_mongo() {
  ...
  mongosh --quiet -u lapnadmin -p "$admin" --authenticationDatabase admin --eval \
    ...
    2>/dev/null || log_warn "Tạo user Mongo có thể đã tồn tại."
...
cmd_db_drop() {
  ...
  if [[ -z "$DB_FORCE" ]]; then
    local c; c="$(ui_ask "Gõ lại tên DB '$dbname' để xác nhận XÓA")"
    [[ "$c" == "$dbname" ]] || die "Không khớp — hủy."
  fi

  # Dump nhanh trước khi xóa.
  mkdir -p "$LAPN_TRASH"
...
SQL
    ...
    mongo)
      ...
      ;;
    redis) log_info "Redis dùng chung — không drop instance. Bỏ qua." ;;
  esac

  state_update '.sites[$d].db = [(.sites[$d].db // [])[] | select(.engine != $e)]' \
    --arg d "$domain" --arg e "$engine"
  audit "OK" "db:drop $domain $engine"
  log_ok "Đã xóa DB $engine của $domain (dump ở $LAPN_TRASH nếu có)."
}
...
cmd_db_console() {
  ...
  [[ -z "$url" ]] && die "Site '$domain' chưa gắn DB."
  log_info "Kết nối: $url"
  case "$url" in
    mysql://*)      mysql "$(_db_url_to_mysql_args "$url")" ;;
    postgresql://*) psql "$url" ;;
    mongodb://*)    mongosh "$url" ;;
    redis://*)      redis-cli -u "$url" ;;
    *) die "Không nhận diện được loại URL." ;;
  esac
}
...
cmd_db_remote() {
  ...
  if [[ -n "$DBR_DEL" ]]; then
    ...
    log_ok "Đã xóa tunnel user $user."
    return 0
  fi
  ...
  # mặc định = --add
  local key="$DBR_KEY"
  [[ -z "$key" ]] && key="$(resolve_input "key" "" --prompt "Public key cho $user (ssh-ed25519 ...)")"
  [[ "$key" =~ ^(ssh-ed25519|ssh-rsa|ecdsa-) ]] || die "Public key không hợp lệ."

  log_step "Tạo SSH tunnel user $user (chỉ forward, không shell)"
  ...
  # Match block: chỉ port-forward tới các port DB localhost, không shell.
...
EOF
  sshd -t || { rm -f "$conf"; die "sshd_config lỗi — đã hủy."; }
  systemctl reload ssh 2>/dev/null || systemctl reload sshd 2>/dev/null
  audit "OK" "db:remote add $user"
  log_ok "Tunnel user $user sẵn sàng."
...
${C_BOLD}Cấu hình Navicat (SSH tunnel)${C_RESET}
  Tab SSH:
    Host        : ${ip}
    Port        : ${ssh_port}
    User name   : ${user}
    Auth method : Private Key (khớp public key đã thêm)
  Tab General (kết nối DB qua tunnel — localhost LÀ server):
    Host        : 127.0.0.1
    Port        : 3306 (MariaDB/MySQL) | 5432 (Postgres) | 27017 (Mongo) | 6379 (Redis)
    User / Pass : DB user per-site (xem .env của site)

Lưu ý: port DB KHÔNG mở ra Internet; chỉ đi trong SSH tunnel.
EOF
}
